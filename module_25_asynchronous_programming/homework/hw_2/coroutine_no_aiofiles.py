...
CATS_WE_WANT = 150
OUT_PATH = Path(__file__).parent / 'cats_no_aiofiles'
    ...
    async with semaphore:
        for attempt in range(1, retries + 1):
            try:
                ...
                await asyncio.to_thread(write_file, image_content, idx)
...
def write_file(content: bytes, file_id:int):
    filepath = OUT_PATH / f"{file_id}.png"
...
def main():
    ...
    duration = end - start
    print("Total images processed:", len([r for r in res if r is None or r is not None]),
          'in {:.4} seconds'.format(duration))
